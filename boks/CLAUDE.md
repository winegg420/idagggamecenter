...
- `_test/motor-test.mjs` — Node başsız test (88 doğrulama: sınıflandırma, gard, faz, puanlama, savunma, kalori, analiz, eşleştirme, uçtan uca + oynanabilirlik regresyonları)
...
- **Nişan noktası + tür öncelikli isabet:** düz yumrukta bilek EKRANDA durur; darbe anında bilek, kol yönünde derinlik ilerlemesi kadar taşınır (`nx/ny`). Eşleştirme iki aşamalıdır: **doğru numaralı pad mesafeden bağımsız** eşleşir (birden fazlaysa en yakını), doğru numara yoksa yakındaki yanlış numaralı pad dar yarıçapla (`PAD_TOLERANS` 1.05 birim) yalnız geri bildirim (titreme) için eşleşir. Ölçülen beceri "doğru yumruğu doğru elle, süresi içinde atmak"tır; 2D bilek konumu piksel hassasiyetinde bir nişan aracı değildir. Pad ömrüne `PAD_TESPIT_PAYI` (0.3 sn) eklenir: darbe olayı gerçek temastan ~100 ms sonra üretilir.
