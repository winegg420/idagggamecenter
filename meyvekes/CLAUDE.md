...
- `_test/` — motor-test.mjs (Node başsız, 43 doğrulama: faz, kesim, statik el, hızlı savurma, combo, gecikme telafisi, iz üretimi, meyve ye, efektler), cekirdek-test.mjs (Node, sanal saat: worker kare zamanlaması / Hz), kilic-test.html + yeme-test.html (kamerasız görsel testler)
...
- **Worker KLASİK tip olmalı (module DEĞİL):** MediaPipe'ın wasm yükleyicisi worker'da `importScripts` kullanır; `{ type: "module" }` ile açılan worker "Module scripts don't support importScripts()" ile HER cihazda ölür ve kod sessizce ~10 Hz ana-thread yedeğine düşer (Eylül 2026'da bulunan kök neden). `takip-worker.js`'e static `import`/`export` ekleme; yalnız dinamik `import()` (yedek: `importScripts` + `vision_bundle.cjs`).
- **rVFC basamaklanması:** 30 fps kamerada "tek kare uçuşta + meşgulse atla" Hz'i 30/15/10'a kilitler (çevrim 34-66 ms → 15 Hz; telefonda ölçülen 13-14 Hz). Çekirdek sonuç gelir gelmez EN TAZE kareyi gönderir ve meşgulken bir sonraki karenin bitmap'ini ÖNCEDEN çıkarır → Hz ≈ min(kamera fps, 1000/çıkarım). Mobilde kare 352 px (480→352 çıkarımı ~yarıya indirir); worker yavaş GPU'da CPU'yu ölçüp hızlı delegeyi tutar. Zamanlama testi: `_test/cekirdek-test.mjs`.
- **Kamera canvas'a KOPYALANMAZ:** `<video>` canvas'ın altında CSS katmanı (`.mk-video`: cover + `scaleX(-1)`, `koordinatHesap` ile aynı eşleme); canvas `alpha: true`, `ciz(..., video=null)`. Karartma `.mk-video-karartma`.
- **Görsel teşhis:** "🖐 el görünmüyor" / "😐 yüz görünmüyor" rozeti + **"xx Hz · worker/GPU"** (algılama frekansı · yol/delege); ana-thread yedeğine düşüldüyse **"ana/CPU ⚠"**. Worker kurulamama nedeni her zaman `console.warn` ile yazılır. Kasma/ıskalama şikâyetinde ilk bakılacak yer burasıdır.
