...
function Atolye(){
  ...
  return <div className="atolye">
    ...
    <main><section className="gosterim"><div className="sahne" ref={alan}/><div className="sahne-baslik"><span>ORTAK GÖVDE / İNSAN</span><h1>Senin karakterin.</h1><p>Çevir, yakından bak, hareket ettir.</p></div>
      ...
      <fieldset><legend>Üst giyim</legend><div className="secimler kiyafetler">{[['tisort','Tişört'],['atlet','Atlet'],['gomlek','Gömlek'],['ceket','Ceket'],['gelinlik','Gelinlik']].map(([v,ad])=><button key={v} aria-pressed={ayar.kiyafet===v} onClick={()=>setAyar(a=>({...a,kiyafet:v,ceket:v==='ceket'}))}>{ad}</button>)}</div></fieldset>
      <fieldset><legend>Alt giyim</legend><div className="secimler">{[['pantolon','Pantolon'],['sort','Şort'],['kapri','Kapri']].map(([v,ad])=><button key={v} aria-pressed={ayar.alt===v} onClick={()=>sec('alt',v)}>{ad}</button>)}</div>{renkler(['#253341','#3a3f4a','#5a4632','#2f4a3a','#6b4a55','#1f2933'],'altRenk','Alt giyim rengi')}</fieldset>
      <fieldset><legend>Ayakkabı</legend><div className="secimler">{[['spor','Spor'],['bot','Bot'],['terlik','Terlik'],['sandalet','Sandalet']].map(([v,ad])=><button key={v} aria-pressed={ayar.ayakkabi===v} onClick={()=>sec('ayakkabi',v)}>{ad}</button>)}</div>{renkler(['#eee7d8','#2b2b30','#c04a34','#3a5a8c','#d9c27a','#8a8f98'],'ayakkabiRenk','Ayakkabı rengi')}</fieldset>
      ...
      <fieldset><legend>Sakal</legend><div className="secimler">{[['yok','Yok'],['biyik','Bıyık'],['keci','Keçi'],['favori','Favori'],['tam','Tam sakal']].map(([v,ad])=><button key={v} aria-pressed={ayar.sakal===v} onClick={()=>sec('sakal',v)}>{ad}</button>)}</div></fieldset>
      {/* Gözlük ve pelerin artık METİN yuva (eskiden kutucuk/boolean'dı). */}
      <fieldset><legend>Gözlük</legend><div className="secimler">{[['yok','Yok'],['gunes','Güneş'],['kare','Kare'],['yuvarlak','Yuvarlak'],['okuma','Okuma'],['spor','Spor']].map(([v,ad])=><button key={v} aria-pressed={ayar.gozluk===v} onClick={()=>sec('gozluk',v)}>{ad}</button>)}</div></fieldset>
      <fieldset><legend>Sırt</legend><div className="secimler">{[['yok','Yok'],['klasik','Pelerin'],['kisa','Kısa pelerin']].map(([v,ad])=><button key={v} aria-pressed={ayar.pelerin===v} onClick={()=>sec('pelerin',v)}>{ad}</button>)}</div></fieldset>
