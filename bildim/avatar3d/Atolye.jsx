...
function Atolye(){
  ...
  return <div className="atolye">
    ...
    <main><section className="gosterim"><div className="sahne" ref={alan}/><div className="sahne-baslik"><span>{tt("ORTAK GÖVDE / İNSAN")}</span><h1>{tt("Senin karakterin.")}</h1><p>{tt("Çevir, yakından bak, hareket ettir.")}</p></div>
      ...
      <fieldset><legend>{tt("Ayakkabı")}</legend><div className="secimler">{[['spor',tt("Spor")],['bot',tt("Bot")],['terlik',tt("Terlik")],['sandalet',tt("Sandalet")]].map(([v,ad])=><button key={v} aria-pressed={ayar.ayakkabi===v} onClick={()=>sec('ayakkabi',v)}>{ad}</button>)}</div>{renkler(['#eee7d8','#2b2b30','#c04a34','#3a5a8c','#d9c27a','#8a8f98'],'ayakkabiRenk',tt("Ayakkabı rengi"))}</fieldset>
      ...
      <fieldset><legend>{tt("Gözlük")}</legend><div className="secimler">{[['yok',tt("Yok")],['gunes',tt("Güneş")],['kare',tt("Kare")],['yuvarlak',tt("Yuvarlak")],['okuma',tt("Okuma")],['spor',tt("Spor")]].map(([v,ad])=><button key={v} aria-pressed={ayar.gozluk===v} onClick={()=>sec('gozluk',v)}>{ad}</button>)}</div></fieldset>
