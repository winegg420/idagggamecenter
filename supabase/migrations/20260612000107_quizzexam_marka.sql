-- ============================================================================
-- MARKA: Bildim! → QuizzExam
--
-- Push bildirimlerinin varsayılan başlığı 'Bildim!' idi; 'QuizzExam' oldu.
-- Tip'e özel başlıklar (⚔️ Meydan okuma! vb.) marka adı içermediği için
-- olduğu gibi korundu.
--
-- ÖNEMLİ NOT — bu dosya neden 073'ten kopyalanmadı:
-- 20260612000073_bildirim_push.sql içindeki sürüm ESKİ. O dosyada CRON_SECRET
-- fonksiyon gövdesine SABİT gömülüydü. Sonradan (20260612000080 ile gelen)
-- sunucu_gizli tablosu + public.gizli_al() kullanılacak biçimde canlıda
-- güncellendi, ama bu değişiklik hiçbir migration dosyasına yazılmamıştı.
-- Bu yüzden aşağıdaki gövde CANLI tanımdan (pg_get_functiondef) alınmıştır;
-- 073 kopyalansaydı güvenlik düzeltmesi geri alınır ve sızmış sır tekrar
-- depoya girerdi. Tek fark: varsayılan başlık.
--
