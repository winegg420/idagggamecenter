-- ============================================================
-- BilgeBot: meydan okumaları kabul eden, soru cevaplayan ve
-- turnuvaya katılan bot oyuncu
-- ============================================================
...
values (
  ...
  'bilgebot@bildim.app', now(),
  '{"provider":"bot","providers":["bot"]}',
  '{"full_name":"BilgeBot"}',
  now(), now()
)
on conflict (id) do nothing;

-- Trigger profili "bilgebot_xxxx" olarak açar; düzgün isim ver
update public.profiles
   set username = 'BilgeBot',
       avatar_url = 'https://api.dicebear.com/9.x/bottts/svg?seed=BilgeBot'
 where id = 'b0b00000-0000-4000-8000-000000000001'
   and username <> 'BilgeBot';

-- ---------- Botun tek tiki: kabul et, cevapla, ilerlet ----------
    ...
    update public.matches
       set durum = 'aktif',
           soru_ids = (select coalesce(array_agg(id), '{}') from
                        (select id from public.questions where aktif order by random() limit 5) s),
           aktif_soru = 0,
           soru_baslangic = now()
     where id = r.id;
  end loop;
...
-- ---------- Turnuva lobisine katıl (en az 1 gerçek oyuncu varsa) ----------

create or replace function public.bot_join_tournament()
returns void
language plpgsql
security definer
set search_path = public
...
-- Bot fonksiyonlarını sadece cron çağırır
revoke execute on function public.bot_oyna() from public, anon, authenticated;
revoke execute on function public.bot_join_tournament() from public, anon, authenticated;

-- ---------- pg_cron ----------

do $$
begin
  -- Bot tiki: 7 saniyede bir
  begin
    perform cron.unschedule('bildim-bot-oyna');
  exception when others then null;
  end;
  perform cron.schedule('bildim-bot-oyna', '7 seconds', 'select public.bot_oyna()');

  -- Turnuva katılımı: her gece 21:55 TSİ (18:55 UTC)
  begin
    perform cron.unschedule('bildim-bot-turnuva');
  exception when others then null;
  end;
  perform cron.schedule('bildim-bot-turnuva', '55 18 * * *', 'select public.bot_join_tournament()');
