...
// Bot zorluk ayarı (tepki gecikmesi + hata payı)
const BOT = {
  TEPKI_MS: 120,
  HATA: 26,          // hedef x'e eklenen rastgele sapma
  VURUS_MESAFE: OYUNCU.VURUS_MENZIL * 0.9,
};

export function botDurumKur() {
  return { hedefX: SAHA.W / 2, sonKarar: -9999, sapma: 0 };
}
...
export function botGirdiHesapla(mac, slot, botDurum) {
  ...
  if (mac.simMs - botDurum.sonKarar > BOT.TEPKI_MS) {
    ...
    let tahminX = top.position.x + top.velocity.x * dusmeSuresi * 2.2;
  ...
  if (mesafe < 190 && dy < -OYUNCU.KAFA_R && top.position.y < SAHA.ZEMIN_Y - 120) {
    g.zipla = true;
  }

  // Vuruş: top menzilde ve rakip kalesi yönünde anlamlıysa
  const topOnumde = Math.sign(dx) === yon || Math.abs(dx) < TOP.R * 2;
  if (mesafe < BOT.VURUS_MESAFE && topOnumde && Math.random() < 0.6) {
    g.vur = true;
  }

  // Acil savunma: top kaleme çok yaklaştıysa her şekilde uzaklaştır
  ...
  if ((rakipSkor > benimSkor || kaleyeMesafe < 250) && Math.random() < 0.02) {
    g.guc = true;
  }
