...
export type EarType = 'yok' | 'yuvarlak' | 'sivri' | 'uzun' | 'sarkik';
export type SnoutType = 'yok' | 'gaga' | 'burun' | 'hortum';

export interface CharacterDef {
  id: string;
  name: string;
  /** UI liste görünümü için emoji */
  emoji: string;
  /** Karakterin komik tanıtım cümlesi */
  bio: string;
  bodyColor: string;
  bellyColor: string;
  earType: EarType;
  snout: SnoutType;
  /** Gövde ölçek çarpanları [genişlik, boy] */
  bodyScale: [number, number];
  hasTail: boolean;
  /** Varsayılan kozmetik kombinasyonu (sabit imza görünüm) */
  defaultCosmetics: CosmeticConfig;
  /** Karakteristik "ah/of" ses perdesi (Hz taban) */
  voicePitch: number;
}

export interface CosmeticConfig {
  hat: string; // 'yok' | 'kep' | 'silindir' | 'ascibone' | 'fedora' | 'tac' | 'kask' | 'bere'
  hatColor: string;
  glasses: string; // 'yok' | 'gunes' | 'yuvarlak' | 'kare' | 'pilot'
  glassesColor: string;
  necklace: string; // 'yok' | 'altin' | 'papyon' | 'atki' | 'kravat'
  necklaceColor: string;
  wristband: string; // 'yok' | 'bant' | 'saat'
  wristbandColor: string;
  hair: string; // 'yok' | 'rasta' | 'sari' | 'kisa' | 'mohawk' | 'peruk'
  hairColor: string;
  mustache: string; // 'yok' | 'pos' | 'ince'
  beard: string; // 'yok' | 'keci' | 'tam'
  top: string; // 'yok' | 'tisort' | 'atlet' | 'gomlek' | 'takim' | 'esofman' | 'onluk' | 'ceket' | 'uzaykiyafeti' | 'pelerin'
  topColor: string;
  shoes: string; // 'yok' | 'spor' | 'topuklu' | 'bot' | 'terlik'
  shoesColor: string;
}
...
export const CHARACTERS: CharacterDef[] = [
  {
    ...
    bio: 'Plajdan yarışa geldi, acelesi yok ama nedense hep önde.',
  ...
  },
  {
    ...
    bio: 'Toplantısı var, yarışı hızlı bitirmesi lazım.',
  ...
  },
  {
    ...
    bio: 'Kas yapmış, havuç yerine protein tozu taşıyor.',
  ...
  },
  {
    ...
    bio: 'Yarışı bitirince herkese mantı sözü verdi.',
  ...
  },
  {
    ...
    bio: 'Konserine yetişmek için yarışıyor, gitarı kulise bıraktı.',
  ...
  },
  {
    ...
    bio: 'Ay\'a gitti geldi, şimdi hedef podyum.',
  ...
  },
  {
    ...
    bio: 'Kendi koşu ritmini kendisi mixliyor.',
  ...
  },
  {
    ...
    bio: 'Sessiz, hızlı... ama her seferinde bir yere takılıyor.',
  ...
  },
  {
    ...
    bio: '"Bu pist satılık olsa çoktan satmıştım" diyor.',
  ...
  },
  {
    ...
    bio: 'Gündüz yarışlarından nefret ediyor ama katılmadan da duramıyor.',
  ...
  },
  {
    ...
    bio: 'Topuklularla koşabildiğini kanıtlamak için burada.',
  ...
  },
  {
    ...
    bio: 'Dalga yoksa yarış var. Rastaları rüzgarda savruluyor.',
  ...
  },
  {
    ...
    bio: 'Yelesi döküldü diye sarı peruk taktı, kimse fark etmedi sanıyor.',
  ...
  },
  {
    ...
    bio: 'Kel ama bıyığıyla gurur duyuyor. Pos bıyık, dolgun özgüven.',
  ...
  },
  {
    ...
    bio: 'Her checkpoint\'te yeni bir dörtlük yazıyor.',
  ...
  },
  {
    ...
    bio: 'Yarışın optimal rotasını hesapladı, uygulamak ayrı konu.',
  ...
  },
  {
    ...
    bio: 'Isınma turu diye pisti üç kez koştu.',
  ...
  },
  {
    ...
    bio: 'Kimin engel koyduğunu her zaman biliyor. Kanıtı da var.',
  ...
  },
  {
    ...
    bio: 'Rampaları kaykay parkı sanıyor, haksız da değil.',
  ...
  },
  {
    ...
    bio: 'Şapkadan tavşan çıkaramıyor çünkü kendisi tavşan.',
...
export function xpForRace(rank: number, playerCount: number): number {
  const base = 20;
